...
class LatentFeaturesModel(object):
    ...
    def __init__(self, alpha, l):
        ...
        self.alpha = alpha
        self.l = l
        self.Q = create_random_matrix(4503, k)
        self.P = create_random_matrix(34789, k)
...
def predict(learner, filename):
    ...
    for t, row in enumerate(open(filename)):
        ...
        i = int(row[1]) - 1
        r = float(row[2])
        error += math.fabs(r - np.dot(learner.Q[i,:], learner.P[x,:]))
        number += 1
    return error/number

def solution(learner):
    for i in range(epoch):
        for t, row in data(train_file):
            learner.update(row)
        print predict(learner, train_file)
